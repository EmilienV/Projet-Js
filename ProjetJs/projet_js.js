...
collection.forEach((i) => {
  ...
  section.innerHTML = `<img src = ${i.picture}> <span class="genre">${i.genre}</span> <span class="name"> ${i.name}</span> <span class="director_year"> ${i.director}, ${i.releaseYear}</span> <span class="cast">Casting: ${i.cast}</span> <div class="div_bin"> <img class="bin" src="images/delete_icon.png"></div>`;
  article.appendChild(section);
  section.setAttribute("class", i.name.replaceAll(" ", "_"));
});
const boutonsSupprimer = document.querySelectorAll(".bin");
boutonsSupprimer.forEach((bouton) => {
  bouton.addEventListener("click", function () {
    const parentParent = bouton.parentNode.parentNode;
    parentParent.remove();
  });
});
